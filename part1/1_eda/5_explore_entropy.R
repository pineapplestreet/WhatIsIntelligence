  ...
  # if n > 1 we need Conditional Entropy
    ...
    p_ngram <- tmp %>%
      ...
      mutate(total=nrow(tmp),
             p_ngram=freq/total) %>%
      filter(freq > 1) %>%
      mutate(n=n, string=ngram) %>%
      select(n, string, freq)
    # %>%
...
res2 = calculate_entropy(input, 13)


book_data <- readRDS(paste0("clean_data/the_call_of_the_wild_words.Rds"))

string_of_interest <- "and break it out and walk off with it for a hundred yards"
...
res = calculate_entropy(input, 9)

check <- readRDS(paste0("1_eda/entropy_results_backup.Rds")) %>%
  filter(mode=="words") %>%
  filter(source=="The Call of the Wild")
